...
const Giving = {
  ...
  add() {
    ...
    document.getElementById('save-g-btn').onclick=()=>{
      ...
      Storage.insert('giving_donations',d); Modal.close(); Toast.success('Gift recorded'); Giving._rerender();
    };
  },
  edit(id) {
    ...
    Modal.open({ title:'Edit Gift', body:this._form(d), width:'500px',
      footer:`<button class="btn btn-outline" onclick="Modal.close()">Cancel</button>
              <button class="btn btn-primary" id="save-g-btn">Save</button>` });
    document.getElementById('save-g-btn').onclick=()=>{
      Storage.update('giving_donations',id,this._collect()); Modal.close(); Toast.success('Updated'); Giving._rerender();
    };
  },
  remove(id) {
    UI.confirm('Delete this donation record?',()=>{ Storage.removeItem('giving_donations',id); Toast.success('Deleted'); Giving._rerender(); });
  },
  addFund() {
    ...
    document.getElementById('save-f-btn').onclick=()=>{
      ...
      Storage.insert('giving_funds',{ name:n, description:document.getElementById('f-desc')?.value?.trim()||'', goal:parseFloat(document.getElementById('f-goal')?.value)||0, color:document.getElementById('f-color')?.value||'blue', active:true });
  ...
  editFund(id) {
    ...
    document.getElementById('save-f-btn').onclick=()=>{
      const n=document.getElementById('f-name')?.value?.trim();
      if(!Validate.check([['f-name', Validate.required(n,'Fund name')]])) return;
      Storage.update('giving_funds',id,{ name:n, description:document.getElementById('f-desc')?.value?.trim()||'', goal:parseFloat(document.getElementById('f-goal')?.value)||0, color:document.getElementById('f-color')?.value||'blue', active:document.getElementById('f-active')?.value==='true' });
