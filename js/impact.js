...
Navigation.register('impact', function render(page) {
  ...
  page.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;">
      ...
      <div class="card">
        ...
        <div style="font-size:.86rem;display:flex;flex-direction:column;gap:8px;padding-top:4px;">
          ${overdueFollowUp > 0 ? `<div style="padding:8px;background:rgba(239,68,68,.07);border-radius:6px;border-left:3px solid var(--red)"><strong style="color:var(--red)">${overdueFollowUp} overdue family follow-up${overdueFollowUp>1?'s':''}</strong><div style="color:var(--text-muted);font-size:.76rem">Check Family Assistance tab</div></div>` : ''}
          ${activeCare > 0 ? `<div style="padding:8px;background:rgba(234,179,8,.07);border-radius:6px;border-left:3px solid var(--yellow)"><strong>${activeCare} open care case${activeCare>1?'s':''}</strong><div style="color:var(--text-muted);font-size:.76rem">Active care ministry records</div></div>` : ''}
          ${lowStockCount > 0 ? `<div style="padding:8px;background:rgba(239,68,68,.07);border-radius:6px;border-left:3px solid var(--orange)"><strong style="color:var(--orange)">${lowStockCount} low-stock pantry item${lowStockCount>1?'s':''}</strong><div style="color:var(--text-muted);font-size:.76rem">Check Food Pantry inventory</div></div>` : ''}
          ${overdueFollowUp===0 && activeCare===0 && lowStockCount===0 ? `<div style="text-align:center;padding:20px;color:var(--text-muted)">✓ No urgent action items</div>` : ''}
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      ${recentEvents.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Event</th><th>Date</th><th>Attendance</th><th>Families</th><th>Items</th></tr></thead><tbody>${recentEvents.map(e=>`<tr><td><strong>${UI.esc(e.name)}</strong><div style="font-size:.74rem;color:var(--text-muted)">${UI.esc(e.type)}</div></td><td>${UI.fmtDate(e.date)}</td><td>${e.actualAttendance||'—'}</td><td>${e.familiesImpacted||'—'}</td><td>${e.itemsCollected||'—'}</td></tr>`).join('')}</tbody></table></div>` : `<div class="empty-state"><div class="empty-state-icon">🌍</div><div class="empty-state-title">No completed outreach events yet</div></div>`}
    </div>
  `;
