...
(function () {
  ...
  function _setSyncTime() {
    if (syncStatus) {
      var t = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      syncStatus.textContent = 'Synced ' + t;
    }
  }

  // Set timestamp after the auto-sync that fires on sign-in (300 ms delay + buffer)
  SupabaseDB.onAuthChange(function (session) {
    if (session) { setTimeout(_setSyncTime, 700); }
    else { if (syncStatus) syncStatus.textContent = ''; }
  });
