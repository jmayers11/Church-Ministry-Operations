...
const Tasks = {
  ...
  add(defaultStatus) {
    ...
    document.getElementById('save-task-btn').onclick = () => {
      ...
      Storage.insert('tasks',d); Modal.close(); Toast.success('Task added'); Tasks._rerender();
    };
  },
  ...
  edit(id) {
    ...
    document.getElementById('save-task-btn').onclick = () => {
      Storage.update('tasks',id,this._collect()); Modal.close(); Toast.success('Updated'); Tasks._rerender();
    };
  },

  move(id, status) {
    Storage.update('tasks', id, { status });
    Tasks._rerender();
  },

  remove(id) {
    UI.confirm('Remove this task?', () => {
      Storage.removeItem('tasks', id); Toast.success('Removed'); Tasks._rerender();
    });
  },
};
window.Tasks = Tasks;
