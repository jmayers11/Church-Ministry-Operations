...
const FamilyAid = {
  ...
  add() {
    ...
    document.getElementById('save-fa-btn').onclick = () => {
      ...
      Storage.insert('family_assistance',d); Modal.close(); Toast.success('Request saved'); FamilyAid._rerender();
    };
  },
  edit(id) {
    ...
    Modal.open({ title:'Edit Request', body:this._form(f), width:'580px',
      footer:`<button class="btn btn-outline" onclick="Modal.close()">Cancel</button>
              <button class="btn btn-primary" id="save-fa-btn">Save</button>` });
    document.getElementById('save-fa-btn').onclick = () => {
      Storage.update('family_assistance',id,this._collect()); Modal.close(); Toast.success('Updated'); FamilyAid._rerender();
    };
  },
  view(id) {
    const f=Storage.findById('family_assistance',id); if(!f) return;
  ...
  remove(id) {
    UI.confirm('Remove this assistance record?', () => {
      Storage.removeItem('family_assistance', id); Toast.success('Removed'); FamilyAid._rerender();
    });
  },
  exportCSV() {
