...
const Prayer = {
  ...
  add() {
    ...
    document.getElementById('save-pr-btn').onclick = () => {
      ...
      Storage.insert('prayer',d); Modal.close(); Toast.success('Prayer request added'); Prayer._rerender();
    };
  },
  ...
  edit(id) {
    ...
    document.getElementById('save-pr-btn').onclick = () => {
      Storage.update('prayer',id,this._collect()); Modal.close(); Toast.success('Updated'); Prayer._rerender();
    };
  },

  remove(id) {
    UI.confirm('Remove this prayer request?', () => {
      Storage.removeItem('prayer',id); Toast.success('Removed'); Prayer._rerender();
    });
  },

  _markAnswered(id) {
    Storage.update('prayer',id,{status:'Answered'});
  ...
  addPraise() {
    ...
    document.getElementById('save-ps-btn').onclick=()=>{
      ...
      Storage.insert('praise_reports',{title,text,submittedBy:document.getElementById('ps-by')?.value.trim()||'',category:document.getElementById('ps-cat')?.value,date:Storage.today()});
  ...
  removePraise(id) {
    UI.confirm('Remove this praise report?', () => {
      Storage.removeItem('praise_reports', id); Toast.success('Removed'); Prayer._rerender();
    });
  },
};
