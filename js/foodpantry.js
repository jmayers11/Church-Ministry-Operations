...
const PantryMgr = {
  ...
  addItem() {
    ...
    document.getElementById('save-pi-btn').onclick = () => {
      ...
      Storage.insert('pantry_inventory',d); Modal.close(); Toast.success('Item added'); PantryMgr._rerender();
    };
  },
  edit(id) {
    ...
    document.getElementById('save-pi-btn').onclick = () => {
      Storage.update('pantry_inventory',id,this._itemCollect()); Modal.close(); Toast.success('Updated'); PantryMgr._rerender();
    };
  },
  adjustQty(id) {
    const i=Storage.findById('pantry_inventory',id); if(!i) return;
    ...
    document.getElementById('adj-btn').onclick = () => {
      ...
      Storage.update('pantry_inventory',id,{qty:newQty});
      Modal.close(); Toast.success(`Quantity updated to ${newQty} ${i.unit}`); PantryMgr._rerender();
    };
  },
  remove(id) {
    UI.confirm('Remove this item from inventory?',()=>{ Storage.removeItem('pantry_inventory',id); Toast.success('Removed'); PantryMgr._rerender(); });
  },
  addDistribution() {
    const body=`
      <div class="form-row">
    ...
    document.getElementById('save-dist-btn').onclick = () => {
      ...
      Storage.insert('foodpantry',d); Modal.close(); Toast.success('Distribution logged'); PantryMgr._rerender();
    };
  },
  editDist(id) {
  ...
  removeDist(id) {
    UI.confirm('Remove this distribution record?', () => {
      Storage.removeItem('foodpantry', id); Toast.success('Removed'); PantryMgr._rerender();
    });
  },
};
