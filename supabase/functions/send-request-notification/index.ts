 ...
 *   REQUEST_NOTIFICATION_FROM   — your Brevo-verified sender email address
 *
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const NOTIFY_FROM    = Deno.env.get("REQUEST_NOTIFICATION_FROM") || "noreply@church.org";
  const NOTIFY_FROM_NAME = "Ministry Portal";

  if (!BREVO_API_KEY || !NOTIFY_TO) {
    console.error("[send-request-notification] Missing env vars: BREVO_API_KEY or REQUEST_NOTIFICATION_TO");
    return new Response(
  ...
  try {
    brevoRes = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender:      { name: NOTIFY_FROM_NAME, email: NOTIFY_FROM },
    ...
    console.error("[send-request-notification] Brevo fetch error:", err);
    return new Response(
  ...
  if (!brevoRes.ok) {
    ...
    console.error("[send-request-notification] Brevo API error:", brevoRes.status, errText);
    return new Response(
  ...
  console.info("[send-request-notification] Email sent. Brevo messageId:", brevoData.messageId, "| Request:", requestId);

  return new Response(
    JSON.stringify({ ok: true, emailId: brevoData.messageId, requestId }),
